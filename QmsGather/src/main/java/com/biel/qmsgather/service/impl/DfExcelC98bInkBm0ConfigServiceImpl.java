...
import com.biel.qmsgather.mapper.DfExcelC98bInkBm0ConfigMapper;
import com.biel.qmsgather.service.DfExcelC98bInkBm0ConfigService;
...
/**
* @author 96901
* @description 针对表【df_excel_c98b_ink_bm0_config】的数据库操作Service实现
* @createDate 2025-02-18 11:57:27
*/
@Slf4j
@Service
public class DfExcelC98bInkBm0ConfigServiceImpl extends ServiceImpl<DfExcelC98bInkBm0ConfigMapper, DfExcelC98bInkBm0Config>
    implements DfExcelC98bInkBm0ConfigService{


    @Autowired
    private DfExcelC98bInkBm0ConfigMapper configMapper;


    @Transactional(rollbackFor = Exception.class)
    public int importConfigFromExcel(MultipartFile file) {
        List<DfExcelC98bInkBm0Config> configList = new ArrayList<>();
        ...
        try (Workbook workbook = WorkbookFactory.create(file.getInputStream())) {
            ...
            if (sheet == null) {
                log.error("未找到BM0工作表");
                throw new RuntimeException("未找到BM0工作表");
            }

            log.info("开始读取Excel配置数据，处理第1-5行");
            ...
            for (int i = 0; i <= 4; i++) {
                ...
                DfExcelC98bInkBm0Config config = new DfExcelC98bInkBm0Config();
                try {
                    // 读取每列数据
    ...
    @Transactional(rollbackFor = Exception.class)
    public int batchSaveConfig(List<DfExcelC98bInkBm0Config> configList) {
        ...
        for (DfExcelC98bInkBm0Config config : configList) {
            try {
    ...
    private boolean isValidConfigData(DfExcelC98bInkBm0Config config) {
